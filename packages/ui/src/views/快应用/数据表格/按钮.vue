            ...
            <tr v-for="(item, index) in items">
                ...
                <td>
                    <n-popover trigger="click">
                        ...
                        <n-grid cols="3" x-gap="20" y-gap="20" class="p-3">
                            <n-gi v-for="(icon, iconName) in icons">
                                <component @click="item.icon=item.icon==iconName?null:iconName" :class="{'cursor-pointer':true, 'primary': iconName==item.icon}" :is='icon' />
                            </n-gi>
                        </n-grid>
                    </n-popover>
                </td>
                <td class="text-center">
                    <n-button circle title="编辑脚本" size="tiny" @click="editBtnScript(item)" tertiary type="primary"><template #icon><n-icon :component="Code" /></template> </n-button>
                    <n-button class="ml-2" circle size="tiny" @click="items.splice(index,1)" tertiary type="error"><template #icon><n-icon :component="Trash" /></template> </n-button>
                </td>
            </tr>
        </tbody>
    </n-table>
