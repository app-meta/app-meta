<template>
    <n-card segmented size="small">
        <template #header>
            <b>后端服务配置</b>
            <span class="h ml-3">首次使用时，请编辑本页内容并保存，仅当 <Tag>部署方式</Tag> 设置为 <Tag>平台部署</Tag> 才能解锁 ⌈上传与部署⌋ 面板</span>
        </template>
...
<script setup>
    ...
    let custom = ref(JSON.stringify(props.bean.custom??{}, null, 4))

    let toSave = () => {
        let d = _raw(props.bean)
        try{
            d.custom = JSON.parse(custom.value)
        }catch(e){
            return M.notice.error(e.message, '额外配置解析失败')
        }
        console.debug(d)
