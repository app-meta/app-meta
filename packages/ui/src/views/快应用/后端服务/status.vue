<template>
    <n-spin :show="!bean.status">
        <n-grid :x-gap="10" :y-gap="20" :cols="4">
            <n-gi>
                <n-alert :bordered="border" :show-icon="false" :type="bean.status=='online'?'success':'default'" title="运行状态">
            ...
            <n-gi>
                <n-alert :bordered="border" :show-icon="false" type="info" title="CPU占用">
                    <n-text class="text-2xl">
                        <template v-if="bean.cpu!=null">{{bean.cpu}} %</template>
                        <template v-else>-</template>
                    </n-text>
                </n-alert>
            </n-gi>
            <n-gi>
                <n-alert :bordered="border" :show-icon="false" type="warning" title="内存占用">
                    <n-text class="text-2xl">{{mem}}</n-text>
                </n-alert>
            </n-gi>
            <n-gi>
                <n-alert :bordered="border" :show-icon="false" type="error" title="最后启动日期">
...
<script setup>
    ...
    const refresh = ()=> RESULT("/page/terminal/overview", {id:props.aid},d=>bean.value=d.data??{status:'stopped'})
    const mem = computed(()=> bean.value.mem? H.filesize(bean.value.mem):"-")
