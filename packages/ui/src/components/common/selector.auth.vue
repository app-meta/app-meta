...
<template>
    ...
    <n-modal v-model:show="show" preset="card" :style="{width: '1000px'}" :mask-closable="false">
        ...
        <n-form class="mt-2" :show-feedback="false" :label-width="100" label-placement="left">
            <n-space vertical size="large">
                ...
                <n-form-item label="按角色授权">
                    <n-select :disabled="popular" v-model:value="roles" :options="roleOpts" multiple filterable clearable placeholder="选择角色" :render-option="renderRole"/>
                </n-form-item>
...
<script setup>
    ...
    const onOk = ()=>{
        let v = popular.value===true? props.all : [..._map(users.value, U), ..._map(departs.value, D), ..._map(roles.value, R)].join(props.split)
